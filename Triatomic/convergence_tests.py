...
import triatomic.environment as env
...
def multiple_run_from_dict(params, variables):
    ...
    env.generate_paths(paths=paths, mol=mol)
    env.generate_filenames(files=files, mol=mol)
    if not os.path.exists(paths.run_psovbr_data + '/' + files.psovbr_lr):
        ...
        make_files.step_one(paths=paths, files=files, mol=mol, pin_options=opts.pin_options)
        ...
        for jtot in variables[0]:
            ...
            for lr, br, jmax in basis_sizes:
                ...
                mol.get_num_angles()
                env.generate_filenames(files=files, mol=mol)
                make_files.step_two(paths=paths, files=files, mol=mol, options=opts)
