...
import tetraatomic_mod.molecule as molecule
import util.environment as environment
import tetraatomic_mod.options as options
...
def create_class(params):
    ...
    j1_max = params['hin_opts']['j1max']
    j2_max = params['hin_opts']['j2max']
    ...
    lr1_length = (params['mol']['Rmin'][0], params['mol']['Rmax'][0], params['mol']['re'][0])
    lr2_length = (params['mol']['Rmin'][1], params['mol']['Rmax'][1], params['mol']['re'][1])
    br_length = (params['mol']['Rmin'][2], params['mol']['Rmax'][2], params['mol']['re'][2])
    num_sinc_fns = params['pin_opts']['num_sinc_fns']
    num_vbr_fns = params['pin_opts']['max_DVR_fns']
    num_lr1_dvr_fns = params['hin_opts']['num_lr1_functions']
    num_lr2_dvr_fns = params['hin_opts']['num_lr2_functions']
    num_br_dvr_fns = params['hin_opts']['num_Br_functions']
    use_spline = params['mol']['use_spline']
    mol = molecule.Molecule(name=name, mass_combo=mass_combo, j_total=j_total, j1_max=j1_max, j2_max=j2_max,
                            permutation=permutation,
                            parity=parity, energy_cutoff=energy_cutoff, mass=mass, lr1_length=lr1_length,
                            lr2_length=lr2_length,
                            br_length=br_length, num_sinc_fns=num_sinc_fns, num_vbr_fns=num_vbr_fns,
                            num_lr1_dvr_fns=num_lr1_dvr_fns, num_lr2_dvr_fns=num_lr2_dvr_fns,
                            num_br_dvr_fns=num_br_dvr_fns,
